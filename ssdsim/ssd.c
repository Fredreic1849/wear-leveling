...
//        printf("%d ", ssd->channel_head[0].chip_head[0].die_head[0].plane_head[0].blk_head[i].isfrozen);
//
//    }
//    printf("\n");
//    for(i = 0; i < ssd->parameter->block_plane; i++){
//        printf("%d ", ssd->channel_head[0].chip_head[0].die_head[0].plane_head[0].blk_head[i].erase_count);
//
//    }
...
struct ssd_info *simulate(struct ssd_info *ssd)
{
    ...
    fprintf(ssd->outputfile,"      arrive           lsn     size ope     begin time    response time    process time    block0_erase_num    block0_utilization    avg_erasenum    blk_invalid_pages     blk_free_pages     plane_erase_num    plane_utilization\n");
    fflush(ssd->outputfile);

    FILE *file = fopen("avar_WL.csv", "w");
    while(flag!=100)
    {

        flag=get_requests(ssd);
        count_request_serviced++;
        if(count_request_serviced % 5000 == 0){

            if(check_variance(calculate_variance(ssd)) == 1){

//                unfreeze_highest_erase_count_blocks(ssd);
//                freeze_highest_erase_count_blocks(ssd, ssd->parameter->frozen_block_num);
//                data_migration_from_freezeblk_to_dram_for_WL(ssd);
//                invalid_data_migration_from_freezeblk_to_dram(ssd);
//                invalid_data_migration_from_freezeblk_to_dram_for_WL(ssd);
            }
//            utilization(ssd);
            printf("%d   %d \n", count_request_serviced, ssd->dram_for_WL->buffer->write_hit);
//            printf("%lf\n", calculate_variance(ssd));

            // 写入均方差到文件中
            for(i = 0; i < ssd->parameter->block_plane; i++){
                fprintf(file, "%.4f ", single_block_utilization(ssd, i));
            }
            fprintf(file, "%d\n", total_erase_count(ssd));



            //写入块擦除次数
//            for(i = 0; i < 64; i++){
//                fprintf(file, "%d ", ssd->channel_head->chip_head->die_head->plane_head->blk_head[i].erase_count);
//            }
//            fprintf(file, "\n");
            var_num++;
            if(var_num == 5000)break;
                /*
                 * rsrch0： 2200 0000
                 * src2_0:  1500 0000
                 * ts_0:    1800 0000
...
void statistic_output(struct ssd_info *ssd)
{
    ...
    fprintf(ssd->outputfile,"buffer write hit rate: %lf\n",(double)ssd->dram->buffer->write_hit/(((double)ssd->dram->buffer->write_miss_hit) + (double)ssd->dram->buffer->write_hit));


    fprintf(ssd->outputfile,"buffer_for_WL read hits: %13d\n",ssd->dram_for_WL->buffer->read_hit);
    fprintf(ssd->outputfile,"buffer_for_WL read miss: %13d\n",ssd->dram_for_WL->buffer->read_miss_hit);

    fprintf(ssd->outputfile,"buffer_for_WL write hits: %13d\n",ssd->dram_for_WL->buffer->write_hit);
    fprintf(ssd->outputfile,"buffer_for_WL write miss: %13d\n",ssd->dram_for_WL->buffer->write_miss_hit);
    fprintf(ssd->outputfile,"buffer_for_WL write hits rate: %lf\n",(double)ssd->dram_for_WL->buffer->write_hit/((double)ssd->dram_for_WL->buffer->write_miss_hit + (double)ssd->dram_for_WL->buffer->write_hit));
    ...
    printf("erase: %13d\n",erase);
    printf("buffer_for_WL write hits rate: %lf\n",(double)ssd->dram_for_WL->buffer->write_hit/((double)ssd->dram_for_WL->buffer->write_miss_hit + (double)ssd->dram_for_WL->buffer->write_hit));
