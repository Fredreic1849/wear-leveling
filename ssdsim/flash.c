...
struct ssd_info * insert2buffer(struct ssd_info *ssd,unsigned int lpn,int state,struct sub_request *sub,struct request *req) {
    ...
    /*
     * 此处先检查是否在buffer_for_WL中
     * 若在，则对ubffer_for_WL更新
     * 若不在，在buffer进行相应的处理
     */
    if (ssd->parameter->wear_leveling == 1){
        buffer_node = (struct buffer_group *) avlTreeFind(ssd->dram_for_WL->buffer, (TREE_NODE *) &key);
        if (buffer_node != NULL) {
            insert2dram_for_WL(ssd, lpn, state, sub, req, 1);
            return ssd;
        } else if (buffer_node == NULL) {
