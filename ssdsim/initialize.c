...
struct ssd_info *initiation(struct ssd_info *ssd)
{
    ...
    ****/
    ...
    ssd->dram_for_WL = (struct dram_for_WL_info *)malloc(sizeof(struct dram_for_WL_info));
    alloc_assert(ssd->dram_for_WL,"ssd->dram_for_WL");
    memset(ssd->dram_for_WL,0,sizeof(struct dram_for_WL_info));
    initialize_dram_for_WL(ssd);
...
struct parameter_value *load_parameters(char parameter_file[30])
{
    ...
    while(fgets(buf,200,fp)){
        ...
        }else if((res_eql=strcmp(buf,"frozen block num")) ==0) {
            sscanf(buf + next_eql, "%d", &p->frozen_block_num);
...
struct dram_for_WL_infoinfo * initialize_dram_for_WL(struct ssd_info * ssd) {
    unsigned int page_num;
    unsigned int DRAM_FOR_WL_SIZE = ssd->parameter->frozen_block_num * 64 * 2048;
    struct dram_for_WL_info *dram_for_WL = ssd->dram_for_WL;
    dram_for_WL->dram_for_WL_capacity = DRAM_FOR_WL_SIZE;
    dram_for_WL->buffer = (tAVLTree *) avlTreeCreate((void *) keyCompareFunc, (void *) freeFunc);
    dram_for_WL->buffer->max_buffer_sector = DRAM_FOR_WL_SIZE / SECTOR; //512

    dram_for_WL->map = ssd->dram->map;
    ...
    dram_for_WL->map->map_entry = ssd->dram->map->map_entry;
